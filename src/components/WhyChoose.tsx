...
export default function WhyChoose() {
  ...
  return (
    <section id="why-choose" className="py-20 bg-[#0A1628] border-t border-white/5 relative">
      <div className="absolute top-[20%] left-[5%] w-[300px] h-[300px] rounded-full bg-[#C9A84C]/5 blur-[120px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {points.map((pt, idx) => (
            <motion.div
              ...
              className="glass p-6 rounded-2xl space-y-4 hover:border-gold/30 hover:scale-[1.02] transition-all relative overflow-hidden group"
            >
              {/* Background gradient on hover */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#C9A84C]/0 via-[#C9A84C]/0 to-[#C9A84C]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

              {/* Icon wrapper */}
              <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center group-hover:bg-gold-light group-hover:border-gold/30 transition-all duration-300">
                {pt.icon}
              </div>

              <h3 className="text-lg font-heading text-white font-bold group-hover:text-gold transition-colors">
                {pt.title}
              </h3>
